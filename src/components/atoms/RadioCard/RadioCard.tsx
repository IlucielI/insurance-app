...
export const RadioCard = forwardRef<HTMLInputElement, RadioCardProps>(
  (
    ...
    const statusBorder = selected
      ? 'border-blue-600 bg-blue-50/40 ring-2 ring-blue-500/10 shadow-xs'
      : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50/50';

    const disabledState = disabled ? 'opacity-50 cursor-not-allowed bg-slate-50 border-slate-200' : 'cursor-pointer';

    return (
      <label
        htmlFor={radioId}
        className={`relative flex items-start p-4 rounded-xl border-2 transition-all ${statusBorder} ${disabledState} ${className}`}
      >
        <input
          ref={ref}
          type="radio"
